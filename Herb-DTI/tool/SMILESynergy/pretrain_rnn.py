...
class Encoder(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size,
        ...
        # 表示生成num_embeddings个具有embedding_dim大小的嵌入向量；
        self.embed = nn.Embedding(input_size, embed_size)
        # 循环神经网络 torch.nn.GRU()
        self.gru = nn.GRU(embed_size, hidden_size, n_layers,
                          dropout=dropout, bidirectional=True)
    def forward(self, src, hidden=None):
        # src: (T,B) T个句子，每个句子有B个单词
        # 输入input的形状shape为（batch_size, Seq_len），batch_size表示样本数（NLP句子数），
        # Seq_len表示序列的长度（每个句子单词个数）；
        # nn.Embedding(input)的输出output具有（batch_size，Seq_len，embedding_dim）的形状大小;
        embedded = self.embed(src)# (T,B,H) 每个单词生成大小为H的向量
