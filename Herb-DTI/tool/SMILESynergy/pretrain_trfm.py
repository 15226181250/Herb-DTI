...
class TrfmSeq2seq(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, n_layers, dropout=0.1): # n_layers=4
        super(TrfmSeq2seq, self).__init__()
        self.in_size = in_size # 45
        self.hidden_size = hidden_size # 256
        self.embed = nn.Embedding(in_size, hidden_size) # (45, 256)    Embedding() 将一句话变成vector
        self.pe = PositionalEncoding(hidden_size, dropout) # (256, 0.1)
        ...
        self.out = nn.Linear(hidden_size, out_size) # (256, 45)
    ...
    def encode(self, src):
        ...
        if batch_size<=100:
            ...
            while ed<batch_size:
                st += 100
                ed += 100
                out = np.concatenate([out, self._encode(src[:,st:ed])], axis=0) # concatenate()实现字符串的连接
            return out
def parse_arguments():
